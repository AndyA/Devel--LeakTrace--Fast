...
int
buffer_init( buffer * b, size_t initsize, size_t growby ) {
  if ( initsize > 0 ) {
...
int
buffer_ensure( buffer * b, size_t minsize ) {
  if ( b->size < minsize ) {
...
int
buffer_ensure_free( buffer * b, size_t minfree ) {
  return buffer_ensure( b, b->used + minfree );
}

int
buffer_append( buffer * b, const void *m, size_t len ) {
  int err;
...
int
buffer_set_used( buffer * b, size_t used ) {
  int err;
