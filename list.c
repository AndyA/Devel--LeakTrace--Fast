...
int
list_append( list * l, const list_ITY p ) {
  l->flags &= ~list_SORTED;
  return buffer_append( l, &p, sizeof( p ) );
}

int
list_build( list * l, const list_ITY v, size_t sz ) {
  int err;
...
long
list_true_diff( list * a, list * b, const void *p,
  ...
  while ( a_p < a_sz || b_p < b_sz ) {
    while ( a_p < a_sz &&
            ( b_p == b_sz || list_CMP( &b_ar[b_p], &a_ar[a_p] ) > 0 ) ) {
      if ( removed ) {
        removed( a_ar[a_p], p );
      }
      a_p++;
    ...
    while ( b_p < b_sz &&
            ( a_p == a_sz || list_CMP( &a_ar[a_p], &b_ar[b_p] ) > 0 ) ) {
      if ( added ) {
        added( b_ar[b_p], p );
      }
      b_p++;
